...
class Game:
    ...
    def update(self):
        ...
        if self.spawno.not_spawn and self.spawnp.not_spawn and self.spawn_block == 0:
        ...
        if self.enable_spawn_b and self.spawnb.not_spawn:
    ...
    def events(self):
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.QUIT:
                self.show_go_screen()
        if pg.sprite.spritecollide(self.player, self.ghost_list, False) or self.spawnr.lpos[-1] == (
    ...
    def get_name(self):
        ...
        while waiting:
            ...
            self.draw_text('Thank you for partcipating in this experiment', 20, YELLOW, WIDTH / 2, 50)
            self.draw_text('Press any key to exit', 20, YELLOW, WIDTH / 2, 70)
            ...
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    ...
                    print(time.time() - self.elapsed_time)
                    with open('scores.txt', 'a') as f:
                        f.write(str(self.player.bits_collected) + ", " +
                                str(round(time.time() - self.elapsed_time, 2)) + "\n")
                    sys.exit()
                    if len(name) <= 10:
                        if event.key == pg.K_a:
                            name.append('a')
    ...
    def show_start_screen(self):
        ...
        self.screen.fill(BLACK)
        arrowPos = 0
        arrowDict = {0: [WIDTH / 2 - 100, HEIGHT * 1 / 5], 1: [WIDTH * 1 / 2 - 125, HEIGHT * 2 / 5],
                     2: [WIDTH * 1 / 2 - 140, HEIGHT * 3 / 5], 3: [WIDTH * 1 / 2 - 165, HEIGHT * 4 / 5]}
